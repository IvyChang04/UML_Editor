...
public class Association_mode extends Mode{
    ...
    public void mouseDragged(MouseEvent m){
        if(starting_point != null){
            Association_line asso = new Association_line(starting_point.x, starting_point.y, m.getX(), m.getY());
            canvas.drag_line = asso;
            canvas.repaint();
        }
    }
    ...
    public void mouseReleased(MouseEvent m){
        if(starting_point != null){
            ...
            if(ending_point != null){
                Super_object asso = new Association_line(starting_point.x, starting_point.y, ending_point.x, ending_point.y);
                canvas.add_line(asso);
                asso.set_port(conn_obj1.get_ports(port1), conn_obj2.get_ports(port2));
                conn_obj1.get_ports(port1).add_lines(asso);
                conn_obj2.get_ports(port2).add_lines(asso);
            }
        }
        ...
        for(int i = 0; i < objects.size(); i++){
            ...
            if(if_inside != null){
                ...
                if(if_inside.equals("inside composite")){
                    obj = obj.get_selected_obj();
                    port_index = Integer.parseInt(obj.inside(p));
                    connect_point.setLocation(obj.get_ports(port_index).port.getCenterX(), obj.get_ports(port_index).port.getCenterY());
                }

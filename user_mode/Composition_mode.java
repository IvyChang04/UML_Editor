...
public class Composition_mode extends Mode{
    ...
    public void mouseDragged(MouseEvent m){
        if(starting_point != null){
            Composition_line compo = new Composition_line(starting_point.x, starting_point.y, m.getX(), m.getY());
            canvas.drag_line = compo;
            canvas.repaint();
        }
    }
    ...
    public void mouseReleased(MouseEvent m){
        if(starting_point != null){
            ...
            if(ending_point != null){
                Super_object compo = new Composition_line(starting_point.x, starting_point.y, ending_point.x, ending_point.y);
                canvas.add_line(compo);
                compo.set_port(conn_obj1.get_ports(port1), conn_obj2.get_ports(port2));
                conn_obj1.get_ports(port1).add_lines(compo);
                conn_obj2.get_ports(port2).add_lines(compo);
            }
        }
